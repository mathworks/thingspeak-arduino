/*
  ...
  Description: Demonstates reading from a public channel which requires no API key (reading from a private channel requires a read API key).
               ...
               status message, location coordinates, and created-at timestamp associated with the latest feed over secured HTTPS connection.
...
// Root Certificate Check, make sure that the root certificate has not expired
